...
interface VercelProject {
  ...
  url?: string;
  alias?: string[];
  customDomains?: Array<{ name: string; [key: string]: any }>;
  gitRepository?: {
    type: string;
    repo: string;
    repoId: number;
...
export default function Home() {
  ...
  // Fetch Vercel projects (optimized with proper error handling and caching)
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        // Add cache-busting only if needed (for development)
        const cacheBuster = process.env.NODE_ENV === 'development' ? `?t=${Date.now()}` : '';
        const response = await fetch(`/api/vercel-projects${cacheBuster}`, {
        ...
        if (error instanceof Error) {
          if (error.name === 'AbortError') {
            ...
            setProjectsError('Network error. Please check your connection.');
          } else {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto tracking-wide leading-relaxed">
              Explore some of our latest projects deployed on Vercel. (This does not include our wordpress or shopify projects)
            </p>
          </div>
          ...
          {projectsLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                <div className="w-16 h-16 bg-blue-900 rounded-full flex items-center justify-center mx-auto mb-4 animate-pulse">
            ...
            <div className="relative">
              ...
              >
                <CarouselContent className="-ml-3 md:-ml-4">
                  {vercelProjects
                    .filter((project) => {
                      // Filter out projects that only have Vercel URLs (test projects)
                      let hasCustomDomain = false;
                      
                      // Check if project has any non-Vercel domains
                      if (project.customDomains && project.customDomains.length > 0) {
                        hasCustomDomain = project.customDomains.some((domain: any) => 
                          domain.name && !domain.name.endsWith('.vercel.app')
                        );
                      }
                      
                      if (!hasCustomDomain && project.targets?.production?.alias && project.targets.production.alias.length > 0) {
                        hasCustomDomain = project.targets.production.alias.some((alias: string) => 
                          !alias.endsWith('.vercel.app')
                        );
                      }
                      
                      if (!hasCustomDomain && project.alias && project.alias.length > 0) {
                        hasCustomDomain = project.alias.some((alias: string) => 
                          !alias.endsWith('.vercel.app')
                        );
                      }
                      
                      // Also filter out the grimo-dev website itself
                      const isGrimoDev = project.name.toLowerCase().includes('grimo') || 
                                       project.name.toLowerCase().includes('grimodev');
                      
                      return hasCustomDomain && !isGrimoDev;
                    })
                    .map((project, index) => {
                    // Extract custom domain with improved logic
                    let customDomain = null;
                    
                    // First priority: Custom domains from the domains API
                    if (project.customDomains && project.customDomains.length > 0) {
                      // Find the first custom domain that's not a Vercel subdomain
                      const customDomainFound = project.customDomains.find((domain: any) => 
                        domain.name && !domain.name.endsWith('.vercel.app')
                      );
                      if (customDomainFound) {
                        customDomain = customDomainFound.name;
                      }
                    }
                    
                    // Second priority: Check production aliases
                    if (!customDomain && project.targets?.production?.alias && project.targets.production.alias.length > 0) {
                      const nonVercelAlias = project.targets.production.alias.find((alias: string) => 
                        !alias.endsWith('.vercel.app')
                      );
                      if (nonVercelAlias) {
                        customDomain = nonVercelAlias;
                      }
                    }
                    
                    // Third priority: Check project-level aliases
                    if (!customDomain && project.alias && project.alias.length > 0) {
                      const nonVercelAlias = project.alias.find((alias: string) => 
                        !alias.endsWith('.vercel.app')
                      );
                      if (nonVercelAlias) {
                        customDomain = nonVercelAlias;
                    ...
                    // Fallback: Use deployment URL if no custom domain found
                    if (!customDomain) {
                      customDomain = project.targets?.production?.url || project.url;
                    }
                    
                    // Construct URLs with proper protocol and www handling
                    let faviconUrl = null;
                    let projectUrl = null;
                    if (customDomain) {
                      // Clean the domain - remove protocol if present
                      let cleanDomain = customDomain.replace(/^https?:\/\//, '');
                      
                      // Ensure www prefix for proper website access
                      if (!cleanDomain.startsWith('www.')) {
                        cleanDomain = `www.${cleanDomain}`;
                      }
                      
                      // Construct URLs with protocol
                      const domainWithProtocol = `https://${cleanDomain}`;
                      faviconUrl = `${domainWithProtocol}/favicon.ico`;
                      projectUrl = domainWithProtocol;
                    }
                    
                    return (
                      <CarouselItem key={project.id} className="pl-3 md:pl-4 pt-5 basis-4/5 sm:basis-1/2 lg:basis-1/3">
                        <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group border border-gray-100 overflow-hidden">
                          {/* Project Header with Favicon */}
                          <div className="p-6 pb-4">
                            <div className="flex items-start gap-4">
                              ...
                              <div className="flex-shrink-0">
                                ...
                                <div className={`w-12 h-12 rounded-lg bg-blue-100 flex items-center justify-center ${faviconUrl ? 'hidden' : 'flex'}`}>
                          ...
                          {projectUrl && (
                            <div className="px-6 pb-4">
                              <a
                                ...
                                className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors duration-300 group/link"
                              >
                                <ExternalLink className="w-4 h-4 group-hover/link:translate-x-0.5 transition-transform duration-300" />
                                {projectUrl ? projectUrl.replace(/^https?:\/\//, '') : customDomain}
                              </a>
            </div>
                          )}
      ...
      <section id="services" className="py-24 bg-gradient-to-br from-gray-50 to-blue-50 section-transition relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="relative">
            ...
            <div className="sm:hidden mt-8">
              <div className="text-center text-sm text-gray-600 mb-3">
                <span className="font-medium">Services</span> • <span className="text-blue-900 font-semibold">{current} of {count}</span>
              </div>
              <div className="flex justify-center gap-2">
